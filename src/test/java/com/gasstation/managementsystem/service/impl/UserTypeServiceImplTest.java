...
class UserTypeServiceImplTest {
    ...
    void delete() throws CustomNotFoundException {
        UserType mockRepository = UserType.builder().id(3).type("CUSTOMER").build();
        UserTypeDTO mockResult = UserTypeDTO.builder().id(3).type("CUSTOMER").build();
        Mockito.when(optionalValidate.getUserTypeById(3)).thenReturn(mockRepository);
        assertEquals(mockResult, userTypeService.delete(3));
    }
}