...
class UserHelperTest {
    ...
    @Test
    void getListStationIdOfOwner() {
        List<Integer> stationIdsMock = Arrays.asList(1, 2, 3, 4, 5);
        User owner = User.builder().build();
        List<Station> stationList = IntStream.range(1, 6).mapToObj(i -> Station.builder().id(i).build()).collect(Collectors.toList());
        owner.setStationList(stationList);
        assertEquals(userHelper.getListStationIdOfOwner(owner), stationIdsMock);
    }
}